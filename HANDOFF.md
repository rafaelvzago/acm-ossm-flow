...
**Machine paths assume:** `/home/rzago` on this Fedora host.
...
- Repo: `/home/rzago/Code/tdc-flp-2026/ossm-multi-cluster` → remote `rafaelvzago/acm-ossm-flow`
...
| Workspace copy | `/home/rzago/Code/tdc-flp-2026/ossm-multi-cluster/HANDOFF.md` |
...
| `ossm-kiali-spoke` | Istio primary 1 + Kiali UI + Perses + Tempo | `https://api.rzago-ossm-multi-cluster-spoke1.servicemesh.rhqeaws.com:6443` |
...
Note: spoke-two hostname is `zago-…` (no leading `r`), not `rzago-…`.
...
|---------|---------------|-----------------|
| spoke | `10.0.191.55` | `10.0.191.40` |
| spoke-two | `10.0.190.88` | `10.0.190.42` |
...
**Why console/Kiali fail from home laptop without VPN:** `*.apps.*` resolves to private AWS IPs (e.g. `10.0.191.127`). Use RHQE VPN / bastion, or work from a host on that network.
...
4. Spoke-two API DNS spelling: `api.zago-ossm-multi-cluster-spoke2…`.  
