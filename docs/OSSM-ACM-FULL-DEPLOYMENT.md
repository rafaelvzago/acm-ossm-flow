...
| `ossm-kiali-spoke` | Istio primary 1 + Kiali + Perses + Tempo | `https://api.spoke.example.com:6443` |
...
|---------|---------|----------------|------------------|
| spoke | network1 | `<SPOKE_HBONE_LB>` | `<SPOKE_SIDECAR_LB>` |
| spoke-two | network2 | `<SPOKE_TWO_HBONE_LB>` | `<SPOKE_TWO_SIDECAR_LB>` |
...
oc login https://api.spoke.example.com:6443 \
  -u=kubeadmin -p='<KUBEADMIN_PASSWORD>' --insecure-skip-tls-verify
oc config rename-context "$(oc config current-context)" ossm-kiali-spoke

# Part 2+ only:
oc login https://api.spoke-two.example.com:6443 \
  -u=kubeadmin -p='<KUBEADMIN_PASSWORD>' --insecure-skip-tls-verify
...
| Item | Value |
| --- | --- |
| Hub API | https://api.hub.example.com:6443 |
| Spoke API | https://api.spoke.example.com:6443 |
| OpenShift | 4.22.5 |
...
| Item | Value |
| --- | --- |
| Spoke-two API | https://api.spoke-two.example.com:6443 |
| Spoke EW HBONE IP | <SPOKE_HBONE_LB> |
| Spoke EW sidecar IP | <SPOKE_SIDECAR_LB> |
| Spoke-two EW HBONE IP | <SPOKE_TWO_HBONE_LB> |
| Spoke-two EW sidecar IP | <SPOKE_TWO_SIDECAR_LB> |
| ISTIO_VERSION | 1.30.1 |
| MESH_ID | mesh1 |
| SPOKE_CLUSTER_NAME | spoke |
...
# Spoke:     HBONE=<SPOKE_HBONE_LB>  Sidecar=<SPOKE_SIDECAR_LB>
# Spoke-two: HBONE=<SPOKE_TWO_HBONE_LB>  Sidecar=<SPOKE_TWO_SIDECAR_LB>
...
| `TEMPO_TENANT` | `mesh1` |
| Remote OTEL Route (deployed) | `otel-remote-spoke-two-istio-system.apps.spoke.example.com` |
...
**VPN required:** `*.apps.*` resolves to private AWS IPs (e.g. `<PRIVATE_APPS_IP>`).
...
4. **Spoke-two API DNS** may not share the same hostname prefix as spoke1 — verify before login.
